...
export default function ToolsSection() {
  ...
  // Group by type and pick latest 4 per type
  const groupedTools = filteredTools.reduce((acc, tool) => {
    if (!acc[tool.type]) acc[tool.type] = [];
    if (acc[tool.type].length < 4) acc[tool.type].push(tool); // only latest 4
    return acc;
  }, {});
  ...
  return (
    <section className="px-6 md:mx-20 py-10 rounded-md bg-white">
      <Link href="/explore">
        <p className="text-center mb-10 text-xl hover:underline underline-offset-2 font-medium">Explore</p>
      </Link>
